...
resource "aws_s3_bucket" "memory" {
  bucket = "${local.name_prefix}-memory-tf-${data.aws_caller_identity.current.account_id}"
...
resource "aws_s3_bucket" "frontend" {
  bucket = "${local.name_prefix}-frontend-tf-${data.aws_caller_identity.current.account_id}"
  tags   = local.common_tags
}
...
resource "aws_iam_role" "lambda_role" {
  name = "${local.name_prefix}-lambda-tf-role"
  tags = local.common_tags
...
resource "aws_lambda_function" "api" {
  ...
  function_name    = "${local.name_prefix}-api-tf"
...
resource "aws_apigatewayv2_api" "main" {
  name          = "${local.name_prefix}-api-gateway-tf"
  protocol_type = "HTTP"
  tags          = local.common_tags
